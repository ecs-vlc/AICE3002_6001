...
\begin{frame}
	...
	\uncover<3->{This is a \textbf{one-sided finite difference}: it replaces a limit with two function evaluations at nearby points.}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item<4-> We therefore say that the one-sided approximation has \textbf{truncation error} $O(h)$.
		\item<5-> Making $h$ smaller initially makes the estimate better.
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		\item<2-> Only a finite, non-uniformly spaced set of real numbers can be represented.
		\item<3-> The gap between adjacent representable values grows with the magnitude of the number.
		\item<4-> If $h$ is too small, the rounded value of $x+h$ can be exactly $x$.
		\item<5-> Even before that happens, subtracting the nearby values $f(x+h)$ and $f(x)$ discards shared leading digits: \textbf{catastrophic cancellation}.
	\end{itemize}

	\begin{block}{The consequence}<6->
...
\begin{frame}
	...
	\begin{itemize}
		\item<2-> A one-sided numerical gradient needs $p+1$ evaluations of $\mathcal{L}$.
		\item<3-> A symmetric numerical gradient needs $2p$ evaluations.
		\item<4-> A model with millions of parameters would therefore need millions of forward passes for \emph{one} gradient step.
		\item<5-> The estimates also contain truncation and roundoff error.
	\end{itemize}

	\begin{alertblock}{So why learn this?}<6->
